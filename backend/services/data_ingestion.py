...
def ingest_csv(csv_path, source_filename="", db=None):
    ...
    if batch:
        db.bulk_save_objects(batch)
        db.commit()
    logger.info(f"Total: {count} records from {source_filename}")
    return count

def ingest_from_url(url, db=None):
    if db is None: db = SessionLocal()
    fn = url.split("/")[-1]
    ...
    try:
        ...
        count = ingest_csv(csv_path, source_filename=fn, db=db)
