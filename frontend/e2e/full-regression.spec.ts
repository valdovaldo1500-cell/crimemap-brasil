...
async function apiGet(request: any, url: string, timeoutOrOpts: any = 60_000): Promise<any> {
  const timeout = typeof timeoutOrOpts === 'number' ? timeoutOrOpts : (timeoutOrOpts?.timeout ?? 60_000);
...
test.describe('API Sanity', () => {
  test('homepage-stats returns valid data', async ({ request }) => {
    const r = await apiGet(request, `${API}/api/homepage-stats`, { timeout: 15_000 });
  ...
  test('filter-options returns data with AND without selected_states', async ({ request }) => {
    ...
    const r1 = await apiGet(request, `${API}/api/filter-options?ultimos_meses=12`, { timeout: 60_000 });
    ...
    // With state
    const r2 = await apiGet(request, `${API}/api/filter-options?selected_states=RS&ultimos_meses=12`, { timeout: 60_000 });
  ...
  test('state-stats returns data for RS, RJ, MG', async ({ request }) => {
    for (const state of ['RS', 'RJ', 'MG']) {
      const r = await apiGet(request, `${API}/api/state-stats?state=${state}&selected_states=${state}&ultimos_meses=12`, { timeout: 60_000 });
      expect(r.ok()).toBeTruthy();
      const d = await r.json();
  ...
  test('location-stats returns data for cities in each state', async ({ request }) => {
    ...
    for (const c of cities) {
      const r = await apiGet(request, `${API}/api/location-stats?municipio=${encodeURIComponent(c.m)}&state=${c.s}&ultimos_meses=12`, { timeout: 60_000 });
      expect(r.ok()).toBeTruthy();
      const d = await r.json();
  ...
  test('location-stats returns data for bairros', async ({ request }) => {
    ...
    for (const bb of bairros) {
      const r = await apiGet(request, `${API}/api/location-stats?municipio=${encodeURIComponent(bb.m)}&bairro=${encodeURIComponent(bb.b)}&state=${bb.s}&ultimos_meses=12`, { timeout: 60_000 });
      expect(r.ok()).toBeTruthy();
      const d = await r.json();
  ...
  test('heatmap endpoints return data', async ({ request }) => {
    // States
    const r1 = await apiGet(request, `${API}/api/heatmap/states?selected_states=RS&ultimos_meses=12`, { timeout: 60_000 });
    expect(r1.ok()).toBeTruthy();
    const d1 = await r1.json();
    ...
    // Municipios (RS bbox)
    const r2 = await apiGet(request, `${API}/api/heatmap/municipios?south=-34&west=-58&north=-27&east=-49&selected_states=RS&ultimos_meses=12`, { timeout: 60_000 });
    expect(r2.ok()).toBeTruthy();
    const d2 = await r2.json();
  ...
  test('tipo filter reduces results', async ({ request }) => {
    const rAll = await apiGet(request, `${API}/api/location-stats?municipio=PORTO+ALEGRE&state=RS&ultimos_meses=12`, { timeout: 60_000 });
    const dAll = await rAll.json();
    const rFiltered = await apiGet(request, `${API}/api/location-stats?municipio=PORTO+ALEGRE&state=RS&tipo=AMEACA&ultimos_meses=12`, { timeout: 60_000 });
  ...
  test('year filter returns different totals for different years', async ({ request }) => {
    const r23 = await apiGet(request, `${API}/api/state-stats?state=RS&selected_states=RS&ano=2023`, { timeout: 60_000 });
    const r24 = await apiGet(request, `${API}/api/state-stats?state=RS&selected_states=RS&ano=2024`, { timeout: 60_000 });
...
test.describe('Share URL Round-trips', () => {
  test('city share URL loads correct data', async ({ request }) => {
    ...
    const r2 = await apiGet(request, `${API}/api/state-stats?state=RS&selected_states=RS&ultimos_meses=12`, { timeout: 60_000 });
    const d2 = await r2.json();
    expect(d2.total).toBe(rsTotal);
  });

  test('share URL with tipo filter preserves filter', async ({ request }) => {
    const r = await apiGet(request, `${API}/api/location-stats?municipio=PORTO+ALEGRE&state=RS&tipo=AMEACA&ultimos_meses=12`, { timeout: 60_000 });
    expect(r.ok()).toBeTruthy();
    const d = await r.json();
  ...
  test('bairro share URL returns data for accented names', async ({ request }) => {
    ...
    const r = await apiGet(request, `${API}/api/location-stats?municipio=PORTO+ALEGRE&bairro=GLORIA&state=RS&ultimos_meses=12`, { timeout: 60_000 });
    expect(r.ok()).toBeTruthy();
    const d = await r.json();
    expect(d.total).toBeGreaterThan(0);
  });
  ...
  test('semester filter works in share URL', async ({ request }) => {
    const r = await apiGet(request, `${API}/api/state-stats?state=RS&selected_states=RS&semestre=2024-S1`, { timeout: 60_000 });
    expect(r.ok()).toBeTruthy();
    const d = await r.json();
    expect(d.total).toBeGreaterThan(0);
    // S1 should be less than full year
    const rFull = await apiGet(request, `${API}/api/state-stats?state=RS&selected_states=RS&ano=2024`, { timeout: 60_000 });
