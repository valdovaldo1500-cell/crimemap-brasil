...
import { fetchCrimeTypes, fetchSemesters, fetchAutocomplete, fetchSexoValues, fetchCorValues, fetchGrupoValues, fetchFilterOptions, submitBugReport, fetchAvailableStates, fetchStateFilterInfo, fetchLocationStats, fetchStateStats, fetchSystemInfo, fetchDataAvailability, searchLocation } from '@/lib/api';
...
const STATE_CENTROIDS_MAP: Record<string, [number, number]> = {
  RS: [-30.03, -51.22], RJ: [-22.91, -43.17], MG: [-19.92, -43.94],
  SP: [-23.55, -46.63], PR: [-25.43, -49.27], SC: [-27.59, -48.55],
...
export default function Home() {
  ...
  const [center, setCenter] = useState<[number,number]>([-23,-48]);
    ...
    <div className="h-dvh overflow-hidden bg-[#0a0f1a] flex flex-col">
        ...
        <div className="max-w-[1800px] mx-auto px-4 py-2 md:py-3 flex items-center justify-between gap-2">
          ...
          <div className="hidden sm:flex items-center gap-1.5">
            ...
            <div className="flex rounded-xl border border-[#1e293b] overflow-hidden" role="group" aria-label="Modo de visualização">
              <button onClick={()=>{setViewMode('choropleth');trackEvent('view_mode_toggle',{mode:'choropleth'});}} aria-label="Visualização em regiões" className={`px-2 py-2 text-xs ${viewMode==='choropleth'?'bg-[#3b82f6] text-white':'bg-[#1a2234] text-[#94a3b8] hover:bg-[#1e293b]'}`}>Regiões</button>
              <button onClick={()=>{setViewMode('dots');trackEvent('view_mode_toggle',{mode:'dots'});}} aria-label="Visualização em pontos" className={`px-2 py-2 text-xs ${viewMode==='dots'?'bg-[#3b82f6] text-white':'bg-[#1a2234] text-[#94a3b8] hover:bg-[#1e293b]'}`}>Pontos</button>
            </div>
            <div className="flex rounded-xl border border-[#1e293b] overflow-hidden" role="group" aria-label="Modo de taxa">
              <button onClick={()=>{setRateMode('rate');trackEvent('rate_toggle',{mode:'rate'});}} aria-label="Taxa por 100 mil habitantes" className={`px-2 py-2 text-xs ${rateMode==='rate'?'bg-[#3b82f6] text-white':'bg-[#1a2234] text-[#94a3b8] hover:bg-[#1e293b]'}`}>/100K</button>
              <button onClick={()=>{setRateMode('absolute');trackEvent('rate_toggle',{mode:'absolute'});}} aria-label="Total absoluto" className={`px-2 py-2 text-xs ${rateMode==='absolute'?'bg-[#3b82f6] text-white':'bg-[#1a2234] text-[#94a3b8] hover:bg-[#1e293b]'}`}>Total</button>
            </div>
        ...
        {showMobileMenu && (
          <div className="sm:hidden border-t border-[#1e293b] p-3 space-y-3 bg-[#111827] max-h-[50vh] overflow-y-auto">
            ...
            <div className="flex flex-wrap gap-2">
              <div className="flex rounded-xl border border-[#1e293b] overflow-hidden">
                <button onClick={()=>{setViewMode('choropleth');trackEvent('view_mode_toggle',{mode:'choropleth'});}} className={`px-3 py-2 text-sm ${viewMode==='choropleth'?'bg-[#3b82f6] text-white':'bg-[#1a2234] text-[#94a3b8]'}`}>Regiões</button>
                <button onClick={()=>{setViewMode('dots');trackEvent('view_mode_toggle',{mode:'dots'});}} className={`px-3 py-2 text-sm ${viewMode==='dots'?'bg-[#3b82f6] text-white':'bg-[#1a2234] text-[#94a3b8]'}`}>Pontos</button>
              </div>
              <div className="flex rounded-xl border border-[#1e293b] overflow-hidden">
                <button onClick={()=>{setRateMode('rate');trackEvent('rate_toggle',{mode:'rate'});}} className={`px-3 py-2 text-sm ${rateMode==='rate'?'bg-[#3b82f6] text-white':'bg-[#1a2234] text-[#94a3b8]'}`}>/100K</button>
                <button onClick={()=>{setRateMode('absolute');trackEvent('rate_toggle',{mode:'absolute'});}} className={`px-3 py-2 text-sm ${rateMode==='absolute'?'bg-[#3b82f6] text-white':'bg-[#1a2234] text-[#94a3b8]'}`}>Total</button>
              </div>
              <div className="flex rounded-xl border border-[#1e293b] overflow-hidden">
                {(['auto','estados','municipios','bairros'] as const).map(v=>(
      ...
      <div className={`flex ${dataWarnings.length > 0 ? 'h-[calc(100dvh-56px-32px)] md:h-[calc(100dvh-80px-32px)]' : 'h-[calc(100dvh-56px)] md:h-[calc(100dvh-80px)]'}`}>
        ...
        <main className="flex-1 relative z-0">
          ...
          <button
            onClick={() => { const entering = !compareMode; setCompareMode(entering); if (!entering) { compareLocationsRef.current = []; setComparePos(null); setComparisonLocations([]); setComparisonStats([]); } if (entering) setSelectedStates([]); trackEvent(entering ? 'comparison_opened' : 'comparison_closed'); }}
            aria-label={compareMode ? 'Desativar comparação' : 'Ativar comparação'}
            className={`absolute top-[100px] md:top-[116px] left-4 z-[1000] flex items-center gap-2 px-4 py-2.5 rounded-full text-xs font-medium shadow-lg transition-colors ${compareMode ? 'bg-[#7c3aed] text-white border border-[#7c3aed]' : 'bg-[#111827]/90 backdrop-blur-xl border border-[#7c3aed]/60 text-[#c4b5fd] hover:bg-[#7c3aed]/20 hover:text-[#e9d5ff]'}`}
          >
