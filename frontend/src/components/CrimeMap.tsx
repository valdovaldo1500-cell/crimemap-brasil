...
interface Props {
  ...
  onStateMenu?: (sigla: string, stateName: string, x: number, y: number) => void;
}
...
export default function CrimeMap({ center, zoom, filters, viewMode = 'dots', rateMode = 'rate', aggregationOverride = 'auto', selectedStates = [], onToggleState, activeFilter, maxGranularity = 'monthly', availableStates = [], compareMode = false, comparisonLocations = [], onCompareSelect, onDetailOpen, onStateMenu }: Props) {
    ...
    l.on('click', async () => {
      ...
      if (onDetailOpenRef.current) {
        ...
        try {
          ...
          const extra_bairros = rawBairroNames?.length
            ? rawBairroNames
            : components?.map(c => c.bairro).filter(b => b !== bairro && b !== 'Bairro desconhecido');
  ...
  const bindStateInteractions = (layer: L.Layer, stateName: string, sigla: string, weight: number, _feature: any, population?: number | null, crimeTypes?: any[]) => {
    ...
    l.on('click', (e: any) => {
      ...
      trackEvent('state_click', { state_name: stateName, crime_count: weight, view_mode: viewMode });
      if (compareModeRef.current && onCompareSelectRef.current) {
        ...
        if (useChoropleth) {
          ...
          geoJsonRef.current = L.geoJSON(mergedGeoData, {
            ...
            onEachFeature: (feature, layer) => {
              ...
              const info = lookup[name];
              if (info) {
                bindInteractions(layer, displayName, info.weight, info.municipio, undefined, undefined, info.population, info.state ?? undefined);
                addPolygonHover(layer);
                const ctr = L.geoJSON(feature).getBounds().getCenter();
                const lbl = L.marker(ctr, {
                  icon: L.divIcon({
          ...
          if (allBairroFeatures.length > 0) {
            ...
            geoJsonRef.current = L.geoJSON(filteredGeoData, {
              ...
              onEachFeature: (feature, layer) => {
                ...
                if (info) {
                  matchedKeys.add(key);
                  bindInteractions(layer, `${displayName}, ${municipio}`, info.weight, municipio, displayName, info.components, info.population, featureState, info.rawBairroNames);
                  addPolygonHover(layer);
                  const ctr = L.geoJSON(feature).getBounds().getCenter();
                  const lbl = L.marker(ctr, {
                    icon: L.divIcon({
